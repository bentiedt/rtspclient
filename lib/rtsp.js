const net = require('net'),
    ...
    WWW_AUTH_REGEX = new RegExp('([a-zA-Z]+)\s*=\s*"?((?<=").*?(?=")|.*?(?=,?\s*[a-zA-Z]+\s*\=)|.+[^=])', "g"),
    STATUS_OK = 200,
    STATUS_UNAUTH = 401,
    RTP_AVP = "RTP/AVP",
...
module.exports.rfc2435 = rfc2435;
...
module.exports.RtspClient = class extends EventEmitter {
    ...
    async connect(url, { keepAlive = true, connection = 'udp' } = { keepAlive: true, connection: 'udp' }) {
        const { hostname, port } = parse(this._url = url);
        if (!hostname) {
        ...
        for (let x = 0; x < media.length; x++) {
            ...
            if (mediaSource.type === "video" && mediaSource.protocol === RTP_AVP && mediaSource.rtp[0].codec === "JPEG") {
            ...
            if (needSetup) {
                ...
                if (connection === "udp") {
                    ...
                    const rtpReceiver = dgram.createSocket({
                        type: "udp4",
                        reuseAddr: true 
                    });
                    this._sockets.push(rtpReceiver);
                    rtpReceiver.on('close', () => {
                        const index = this._sockets.indexOf(rtpReceiver);
                        if (index > -1) {
                            this._sockets.splice(index, 1);
                        }
                    });
                    rtpReceiver.on('error', (e) => {
                        const index = this._sockets.indexOf(rtpReceiver);
                        if (index > -1) {
                            this._sockets.splice(index, 1);
                        }
                    ...
                    const rtcpReceiver = dgram.createSocket({
                        type: "udp4",
                        reuseAddr: true 
                    });
                    this._sockets.push(rtcpReceiver);
                    rtcpReceiver.on('close', () => {
                        const index = this._sockets.indexOf(rtcpReceiver);
                        if (index > -1) {
                            this._sockets.splice(index, 1);
                        }
                    });
                    rtcpReceiver.on('error', (e) => {
                        const index = this._sockets.indexOf(rtcpReceiver);
                        if (index > -1) {
                            this._sockets.splice(index, 1);
                        }
    ...
    async close(isImmediate = false) {
        ...
        this._sockets.forEach(s => {
            try {
                ...
                this.emit("log", "Error disconnecting UDP socket: " + e);
            }
        });
        this._sockets = [];
        this._client.end();
